...
const ArtworkViewModal = React.memo(
    ({
        ...
        return createPortal(
            <div
                className="fixed inset-0 z-[220] flex items-center justify-center bg-black/90 p-8 backdrop-blur-xl sm:p-12">
                ...
                >
                    <div
                        className="h-full w-full overflow-hidden rounded-[28px] border border-white/10 bg-black/24 shadow-[0_32px_128px_rgba(0,0,0,0.8)]">
                        <img
                            src={src}
                            alt={title}
                            loading="eager"
                ...
                <div
                    className="pointer-events-none absolute bottom-8 left-1/2 z-10 w-[min(560px,calc(100vw-3rem))] -translate-x-1/2 px-3">
                    <div
                        className="mx-auto flex w-fit max-w-full flex-col items-center gap-0.5 rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-center shadow-[0_18px_60px_rgba(0,0,0,0.42)] backdrop-blur-xl">
...
export const TrackColumn = React.memo(({track, maxArt}: { track: Track; maxArt?: string }) => {
    ...
    return (
        <div
            className="flex h-full min-h-0 w-full flex-col items-center justify-center gap-[clamp(8px,1.4vh,22px)] overflow-y-auto scrollbar-hide px-12 py-6">
            <div
            ...
            >
                {artwork500 ? (
                    <>
                        ...
                        >
                            <div
                                className="flex flex-col items-center gap-2 scale-90 group-hover/art:scale-100 transition-transform duration-300">
                                <div
                                    className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center border border-white/20">
                                    <Eye size={24}/>
                                </div>
                  ...
                  {t('track.viewArtwork')}
                    ...
                    <div
                        className="w-full h-full bg-gradient-to-br from-white/[0.06] to-white/[0.02] flex items-center justify-center">
            ...
            <div className={`${widthClass} text-center space-y-1`}>
                <div className="flex items-center justify-center gap-2 min-w-0">
                    ...
                    {track.access === 'preview' && (
                        <span
                            className="shrink-0 text-[9px] font-semibold uppercase tracking-wide bg-amber-500/20 text-amber-400/90 px-1.5 py-px rounded">
              Preview
            </span>
                    )}
                </div>
