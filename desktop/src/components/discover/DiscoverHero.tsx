...
const MetaPill = memo(function MetaPill({
  ...
  if (loading) {
    return <Skeleton className="h-3 w-16 rounded-full" />;
  }
  return (
  ...
  return (
    ...
    >
      ...
      {loading || value == null ? (
        ...
        <span className="text-[22px] font-black tabular-nums tracking-tight text-white">
          {fc(value)}
        </span>
      )}
...
const CompassArtifact = memo(function CompassArtifact({ aura }: { aura: Aura }) {
  return (
    <div className="relative shrink-0 self-center lg:self-start w-[180px] h-[180px] md:w-[220px] md:h-[220px]">
      ...
      >
        <div
          ...
          style={{
            background:
              'radial-gradient(80% 60% at 50% 0%, rgba(255,255,255,0.18) 0%, transparent 60%)',
